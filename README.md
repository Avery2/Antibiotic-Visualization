...
Note that the x-axis bacteria labels on the shared x-axis of these charts are symbols that correspond to a key. This was because each bacteria label repeats three times, and they are not in order. Rather than rotate the entire visual or write text vertically, I found symbols, while not ideal (tool-tips might be better), were the best choice here.

The third visual is a bar chart that encodes the counts of the relative MIC variable for each antibiotic. This encodes the count of each relative MIC score per antibiotic through length and positioning, rather than requiring actual counting in the first and second visuals.
